...
let UserResolver = class UserResolver {
    changePassword({ redis, req }, password, token) {
    ...
    forgotPassword({ redis }, email) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield redis.set(constants_1.FORGET_PASSWORD_PREFIX + token, user.id, "ex", 1000 * 60 * 10);
            yield sendEmail_1.sendEmail(email, `<a href="http://localhost:3000/change-password/${token}"> reset paswword </a> `);
            return true;
        });
    }
    me({ req }) {
    ...
    login(credentials, { req }) {
        return __awaiter(this, void 0, void 0, function* () {
            const user = yield User_1.User.findOne({ where: { email: credentials.email } });
            if (!user) {
                return {
                    errors: [
                        {
